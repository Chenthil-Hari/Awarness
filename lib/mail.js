...
export const sendSupportReplyEmail = async (to, name, originalQuery, replyMessage) => {
  const html = `
    <!DOCTYPE html>
    <html>
    <head>
      ...
      <style>
        body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f8fafc; margin: 0; padding: 0; }
        .container { max-width: 600px; margin: 20px auto; background-color: #ffffff; border-radius: 20px; overflow: hidden; box-shadow: 0 10px 25px rgba(0,0,0,0.05); border: 1px solid #e2e8f0; }
        .header { background: #7c3aed; padding: 40px 20px; text-align: center; color: white; }
        .content { padding: 40px 30px; line-height: 1.8; color: #1e293b; font-size: 16px; }
        .footer { padding: 25px; text-align: center; font-size: 11px; color: #94a3b8; background-color: #f8fafc; border-top: 1px solid #e2e8f0; }
        .badge { display: inline-block; padding: 4px 12px; border-radius: 20px; background: rgba(0,0,0,0.1); color: white; font-size: 12px; font-weight: 700; margin-bottom: 15px; text-transform: uppercase; letter-spacing: 1px; }
        .query-box { background: #f8fafc; padding: 20px; border-radius: 12px; border: 1px solid #e2e8f0; margin: 20px 0; font-size: 14px; color: #64748b; }
        .reply-box { background: #f1f5f9; padding: 25px; border-radius: 12px; border-left: 4px solid #7c3aed; margin: 20px 0; font-weight: 500; }
        .button { display: inline-block; padding: 14px 28px; background-color: #1e293b; color: white; text-decoration: none; border-radius: 12px; font-weight: bold; margin-top: 20px; }
      </style>
    </head>
    <body>
      <div class="container">
        <div class="header">
          <div class="badge">SUPPORT RESOLUTION</div>
          <h1 style="margin:0; font-size: 26px;">Mission Briefing: Support Ticket</h1>
        </div>
        <div class="content">
          <p>Hello <strong>${name}</strong>,</p>
          <p>The <strong>Awareness Pro Command Center</strong> has reviewed your recent query.</p>
          
          <div style="font-size: 12px; font-weight: 800; color: #94a3b8; text-transform: uppercase; margin-bottom: -10px;">Your Original Query:</div>
          <div class="query-box">
            "${originalQuery}"
          </div>

          <div style="font-size: 12px; font-weight: 800; color: #7c3aed; text-transform: uppercase; margin-bottom: -10px;">Administrative Response:</div>
          <div class="reply-box">
            ${replyMessage.replace(/\n/g, '<br>')}
          </div>
          
          <p>If you need further clarification, please do not hesitate to reach out again.</p>
          
          <div style="text-align: center; margin-top: 35px;">
            <a href="${process.env.NEXTAUTH_URL}/profile" class="button">View Ticket Status</a>
          </div>
          
          <p style="margin-top: 40px; font-size: 14px;">Safe training,<br><strong>The Awareness Pro Support Team</strong></p>
        </div>
        <div class="footer">
          &copy; 2026 Awareness Pro Global Operations. All rights reserved. <br>
          Sent via Titan Mail Security • Support Thread #${Math.floor(Math.random() * 100000)}
        </div>
      </div>
    </body>
    </html>
  ...
  try {
    const info = await transporter.sendMail({
      from: `"Awareness Pro Support" <${process.env.EMAIL_SERVER_USER}>`,
      to,
      subject: "🛡️ Support Briefing: Your query has been answered",
      html,
    });
    return info;
  } catch (error) {
...
export const sendBroadcastEmail = async (to, name, subject, message, type = 'announcement') => {
  const isMaintenance = type === 'maintenance';
  const accentColor = isMaintenance ? '#f59e0b' : '#7c3aed';
  const icon = isMaintenance ? '⚙️' : '📢';
  ...
  const html = `
    ...
    <head>
      ...
      <style>
        body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f8fafc; margin: 0; padding: 0; }
        .container { max-width: 600px; margin: 20px auto; background-color: #ffffff; border-radius: 20px; overflow: hidden; box-shadow: 0 10px 25px rgba(0,0,0,0.05); border: 1px solid #e2e8f0; }
        .header { background: ${accentColor}; padding: 40px 20px; text-align: center; color: white; }
        .content { padding: 40px 30px; line-height: 1.8; color: #1e293b; font-size: 16px; }
        .footer { padding: 25px; text-align: center; font-size: 11px; color: #94a3b8; background-color: #f8fafc; border-top: 1px solid #e2e8f0; }
        .badge { display: inline-block; padding: 4px 12px; border-radius: 20px; background: rgba(0,0,0,0.1); color: white; font-size: 12px; font-weight: 700; margin-bottom: 15px; text-transform: uppercase; letter-spacing: 1px; }
        .message-box { background: #f1f5f9; padding: 25px; border-radius: 12px; border-left: 4px solid ${accentColor}; margin: 25px 0; font-style: italic; }
        .button { display: inline-block; padding: 14px 28px; background-color: #1e293b; color: white; text-decoration: none; border-radius: 12px; font-weight: bold; margin-top: 20px; }
      </style>
    </head>
    <body>
      <div class="container">
        <div class="header">
          <div class="badge">SYSTEM DISPATCH</div>
          <h1 style="margin:0; font-size: 26px;">${icon} ${subject}</h1>
        </div>
        <div class="content">
          <p>Hello <strong>${name}</strong>,</p>
          <p>This is an official communication from the <strong>Awareness Pro Command Center</strong>.</p>
          
          <div class="message-box">
            ${message.replace(/\n/g, '<br>')}
          </div>
          
          <p>If you have any questions regarding this dispatch, please reach out to our support team through your dashboard inbox.</p>
          
          <div style="text-align: center; margin-top: 35px;">
            <a href="${process.env.NEXTAUTH_URL}" class="button">Access Command Center</a>
          </div>
          
          <p style="margin-top: 40px; font-size: 14px;">Stay vigilant,<br><strong>The Awareness Pro Admin Team</strong></p>
        </div>
        <div class="footer">
          &copy; 2026 Awareness Pro Global Operations. All rights reserved. <br>
          Sent via Titan Mail Security • This is an administrative broadcast.
        </div>
      </div>
    </body>
    </html>
  ...
  try {
    const info = await transporter.sendMail({
      from: `"Awareness Pro Dispatch" <${process.env.EMAIL_SERVER_USER}>`,
      to,
